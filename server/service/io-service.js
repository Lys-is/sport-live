   ...
   start(serv) {
      ...
      io.on("connection", async (socket) => {
         try {
            ...
            const userToken = await tokenService.validateRefreshToken(
...
function startPanel(controls, socket) {
   ...
   if (socket.user) {
      ...
      control =
         controls[userId] ||
         new controlService(userId, socket.user.tablo_style, io);
   ...
   socket.on("get_comm_judge_img", async () => {
      try {
         let data = {};
         const user_id = userId.split("_")[0];

         data.commentators =
            (await Commentator.find({ creator: user_id })) || [];
         data.judges = (await Judge.find({ creator: user_id })) || [];
