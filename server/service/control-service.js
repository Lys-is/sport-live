...
function pauseTimer(timerId) {
   if (timerId) timers.pauseInterval(timerId);
}
function resumeTimer(timerId) {
   if (timerId) timers.resumeInterval(timerId);
}
...
function deleteTimer(timer) {
   if (timer.timerId) timers.clearInterval(timer.timerId);
   timer.timerId = null;
}
class Timer {
   constructor() {
   ...
   set changeStage(stages) {
      ...
      this.now_penalty = stages.now_penalty
         ? stages.now_penalty
         : this.now_penalty;
   }
   ...
   get timeData() {
      ...
      if (this.seconds < 0) {
         ...
         this.seconds = Math.abs(
            60 * Math.floor(this.seconds / 60) - this.seconds
         );
      }
      if (this.seconds > 59) {
         this.minuts += Math.floor(this.seconds / 60);
...
class Scoreboard {
   ...
   changeScore(score) {
      for (let key in score) {
         score[key] =
            typeof score[key] == "object" || !score[key]?.toString
               ? score[key]
               : score[key].toString();
      }
      ...
      this.team1_color = score.team1_color
         ? score.team1_color
         : this.team1_color;
      this.team2_color = score.team2_color
         ? score.team2_color
         : this.team2_color;
      this.penalty = score.penalty ? score.penalty : this.penalty;
      this.team1_font_color =
         getBrightness(this.team1_color) > 0.5 ? "black" : "white";
      this.team2_font_color =
         getBrightness(this.team2_color) > 0.5 ? "black" : "white";
   }
}
class Tablo {
   constructor() {
...
class Control {
   ...
   async setMatch(matchId) {
      ...
      let activePlayers = await PlayerResult.find({
         match: matchId,
         is_active: true,
      }).select("player team");
      // console.log(activePlayers);
      let players_1 = activePlayers
            .filter(
               (el) => el.team._id.toString() == match.team_1._id.toString()
            )
            .map((el) => ({
               fio: el.player.fio,
               _id: el.player._id,
               img: el.player.img,
               num: el.player.num,
            })), //await Player.find({team: match.team_1._id}).select('fio _id img'),
         players_2 = activePlayers
            .filter(
               (el) => el.team._id.toString() == match.team_2._id.toString()
            )
            .map((el) => ({
               fio: el.player.fio,
               _id: el.player._id,
               img: el.player.img,
               num: el.player.num,
            })), //await Player.find({team: match.team_2._id}).select('fio _id img'),
         couch_1 = await Couch.find({
            team: match.team_1._id,
            status_doc: { $ne: "deleted" },
         }).select("fio _id img"),
         couch_2 = await Couch.find({
            team: match.team_2._id,
            status_doc: { $ne: "deleted" },
         }).select("fio _id img"),
         representative_1 = await Representative.find({
            team: match.team_1._id,
         }),
         representative_2 = await Representative.find({
            team: match.team_2._id,
         });
      ...
      this.scoreboard.changeScore({
         team1_color: match.team_1.color,
         team2_color: match.team_2.color,
      });
   ...
   get getData() {
      return JSON.parse(
         JSON.stringify(this, function replacer(key, value) {
            return key.includes("img") ? undefined : value;
         })
      );
   }
   get getDataAll() {
      ...
      function recurse(data, obj) {
         ...
         for (let key in obj) {
            console.log(key);
            if (key.includes("img")) data[key] = "cached";
            else {
