...
class CommentatorController {
   ...
   async post__create(req, res) {
      try {
         ...
         if (!commentator)
            return res.json({ message: "Комментатор не создан, ошибка" });
   ...
   async get__edit(req, res) {
      try {
         ...
         return sendRes(
            "partials/lk_part/commentator_edit",
            { commentator, teams },
            res
         );
      } catch (e) {
   ...
   async put__edit(req, res) {
      try {
         ...
         if (!commentator)
            return res.json({ message: "Такого комментатора не существует" });
         commentator = await Commentator.updateOne(
            { _id: commentatorId },
            req.body
         );
         return res.json({
            message: "Комментатор обновлен",
            redirect: `lk?page=commentator`,
         });
      } catch (e) {
         console.log(e);
         return res.json({ message: "Произошла ошибка" });
...
export default new CommentatorController();
