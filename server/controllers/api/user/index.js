...
class UserController {
   async put__edit(req, res) {
      try {
         ...
         if (user.isAdmin)
            return res.json({ message: "Нельзя изменять администратора" });
         let subscribe = await Subscribe.findOne({ creator: userId });
         console.log(subscribe);
         if (type == "unblock") {
            if (subscribe) await Subscribe.deleteOne({ _id: subscribe._id });
            ...
            user = await User.updateOne(
               { _id: userId },
               { isActive: true, dateActive: formatDate() }
            );
         } else if (type == "block") {
            await Subscribe.deleteOne({ _id: subscribe._id });
            user = await User.updateOne(
               { _id: userId },
               { isActive: false, dateActive: "" }
            );
         } else if (type == "change_block") {
            if (!req.body.new_date) return res.json({ message: "Нет даты" });
            if (subscribe) {
   ...
   async delete__user(req, res) {
      try {
         ...
         if (!user)
            return res.json({ message: "Такого пользователя не существует" });
         if (user.isAdmin)
            return res.json({ message: "Нельзя удалить администратора" });
