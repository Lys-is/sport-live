...
document.addEventListener("DOMContentLoaded", (e) => {
   ...
   modals.forEach((el) => {
      el.addEventListener("click", (e) => {
         if (
            !e.target.closest(".modal_wrap") ||
            e.target.closest(".close_modal")
         )
...
window.onload = function () {
   ...
   dataInpts.forEach((inp) => {
      console.log(inp);
      if (inp.value == "") {
         inp.value = new Date().toLocaleDateString({
...
function input_file_text(inp) {
   ...
   if (file) span.innerHTML = file.name;
}
...
async function getNotification() {
   ...
   if (notificationForm) notification_messages.innerHTML = notificationForm;
}
         ...
         searchInpts.forEach((inp) => {
            ...
            inp.addEventListener("input", (e) => {
               ...
               searchElems.forEach((elem) => {
                  ...
                  if (box.getAttribute("data-searchChecked") == value) return;
...
function getBase64(file) {
   ...
   if (file.size > max_size) {
      alert(
         `Файл слишком большой. Максимальный размер - ${(
            max_size /
            (1024 * 1024)
         ).toFixed(3)} МБ`
      );
