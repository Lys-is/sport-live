...
let cacheHomeScore = 0;
let cacheAwayScore = 0;
...
socket.on("connect", async () => {
   ...
   const res = await socket.emit("get_comm_judge_img");
   console.log(res);

   socket.on("timer_test", (data) => {
      console.error("TIMER TEST");
      console.log(data);
   });
});
...
socket.on("new_notify", (data) => {
   ...
   for (let i = 0; i < data.ids.length; i++) {
      ...
      if (data.playerNum)
         newNotify = newNotify.replace(
            "{{num}}",
            data.playerNum[i] != "null" ? data.playerNum[i] : ""
         );
      else newNotify = newNotify.replace("{{num}}", "");
      if (data.teamD)
         newNotify = newNotify.replace(
            "{{team_logo}}",
            `bg_${data.teamD[i]}_logo`
         );
      else newNotify = newNotify.replace("{{team_logo}}", ``);
      let img = globalImgs.get(data.ids[i]);

      if (img) newNotify = newNotify.replace("{{img}}", img);
      else
         newNotify = newNotify.replace(
            "{{img}}",
            "/static/styles/icons/empty.png"
         );
      newNotify = newNotify.replace("{{main_class}}", data.type || "");
      if (data.add_class && data.add_class[i])
         newNotify = newNotify.replace("{{add_class}}", data.add_class[i]);
...
function setData(data) {
   ...
   if (data.scoreboard) {
      ...
      if (cacheHomeScore < Number(scoreboard.team1)) {
         cacheHomeScore = Number(scoreboard.team1);
         panelTop.classList.remove("away-goal");
         panelTop.classList.add("home-goal");
         window.setTimeout(() => panelTop.classList.remove("home-goal"), 4000);
      }

      if (cacheAwayScore < Number(scoreboard.team2)) {
         cacheAwayScore = Number(scoreboard.team2);
         panelTop.classList.remove("home-goal");
         panelTop.classList.add("away-goal");
         window.setTimeout(() => panelTop.classList.remove("away-goal"), 4000);
      }
   ...
   if (data.match && data.cached) {
      getA(".home-logo").forEach((el) => {
         el.src = data.match.team_1.img
            ? data.match.team_1.img
            : "/static/styles/icons/logo.jpg";
      });
      getA(".away-logo").forEach((el) => {
         el.src = data.match.team_2.img
            ? data.match.team_2.img
            : "/static/styles/icons/logo.jpg";
      });
      ...
      if (!data.match.circle || data.match.circle == "0") {
         ...
         getA(".var-tour").forEach(
            (el) => (el.innerHTML = data.match.circle || 0)
         );
         getA("#tour").forEach((el) => (el.style.display = ""));
      }
      getA(".var-match-date, .var-match-date-text").forEach((el) => {
         el.innerHTML = date;
      });
      getA(".var-match-time, .var-match-time-text").forEach((el) => {
         el.innerHTML = data.match.time || "00:00";
      });
      getA(".var-tournament").forEach(
         (el) =>
            (el.innerHTML =
               data.match.tournament?.basic.full_name || "Вне турнира")
      );
      getA(".var-stadium").forEach(
         (el) => (el.innerHTML = data.match.stadium || "Стадион не указан")
      );
      socket.emit("get_tour_img");
      let arrAllmodels = data.players_1.concat(
         ...data.players_2,
         ...data.couch_1,
         ...data.couch_2,
         ...data.representative_1,
         ...data.representative_2,
         data.match.team_1,
         data.match.team_2
      );
...
function clearData() {
   let classes =
      ".home-couch, .away-couch, .home-representative, .away-representative";
   ...
   arr.forEach((el, i) => {
      gridDiv.innerHTML += `<div class="roster-teamate">
                                ...
                                <img width="40" height="40" src="${
                                   el.img || "/static/styles/icons/empty.png"
                                }">
                            </div>`;
   });
}
...
function setTime(data) {
   ...
   if (!data.value) data.value = `${min}:${sec}`;
   Object.keys(timerDivs).forEach((key) => {
      if (key == "t") {
         timerDivs[key]?.forEach((el) => (el.innerHTML = data.value));
      } else if (key == "r") {
         timerDivs[key]?.forEach(
            (el) =>
               (el.innerHTML =
                  data.scenarios == "1" || data.scenarios == "2"
                     ? `${data.scenarios}T`
                     : "")
         );
      } else if (key == "tr") {
         timerDivs[key]?.forEach(
            (el) =>
               (el.innerHTML =
                  data.value + (data.scenarios == "1" || data.scenarios == "2")
                     ? `  ${data.scenarios}T`
                     : "")
         );
      } else if (key == "nr") {
         timerDivs[key]?.forEach(
            (el) => (el.innerHTML = scenariosA[data.scenarios])
         );
      } else if (key == "n") {
         ...
         else timerDivs[key]?.forEach((el) => (el.innerHTML = ""));
      } else if (key == "rs") {
         timerDivs[key]?.forEach(
            (el) =>
               (el.innerHTML =
...
async function setAnim(div, direct, type) {
   ...
   let [prevDisplay, nextDisplay] =
      direct == "normal" ? ["none", visibleType] : [visibleType, "none"];
...
function setScoreboard(scoreboard) {
   ...
   getA(".var-homeColor").forEach((el) => {
      el.style.background = scoreboard.team1_color;
      el.style.color = scoreboard.team1_font_color;
   });
   getA(".var-awayColor").forEach((el) => {
      el.style.background = scoreboard.team2_color;
      el.style.color = scoreboard.team2_font_color;
   });
   getA(".var-homeTextColor").forEach(
      (el) => (el.style.color = scoreboard.team1_font_color)
   );
   getA(".var-awayTextColor").forEach(
      (el) => (el.style.color = scoreboard.team2_font_color)
   );
   getA(".var-home-fouls").forEach(
      (el) => (el.innerHTML = scoreboard.team1_foll)
   );
   getA(".var-away-fouls").forEach(
      (el) => (el.innerHTML = scoreboard.team2_foll)
   );
   getA(".bgleft").forEach(
      (el) => (el.style.borderBottomColor = scoreboard.team1_color)
   );
   getA(".bgright").forEach(
      (el) => (el.style.borderBottomColor = scoreboard.team2_color)
   );
...
function setPenalty(penalty) {
   ...
   penalty.forEach((el, i) => {
      team1.forEach((div) => {
         div.innerHTML += penaltyHTML.replace(
            "{{penalty.type}}",
            el.team1 ? el.team1 : "clear"
         );
      });
      team2.forEach((div) => {
         div.innerHTML += penaltyHTML.replace(
            "{{penalty.type}}",
            el.team2 ? el.team2 : "clear"
         );
      });
   });
   scorePen1.forEach(
      (div) =>
         (div.innerHTML =
            penalty.filter((el) => el.team1 == "goal").length || 0)
   );
   scorePen2.forEach(
      (div) =>
         (div.innerHTML =
            penalty.filter((el) => el.team2 == "goal").length || 0)
   );
...
function setNames(data) {
   getA(".var-home, .home-team, .var-home-short").forEach(
      (el) => (el.innerHTML = data.team1_name)
   );
   getA(".var-away, .away-team, .var-away-short").forEach(
      (el) => (el.innerHTML = data.team2_name)
   );

   getA(".var-home-three").forEach(
      (el) => (el.innerHTML = data.team1_name.slice(0, 3))
   );
   getA(".var-away-three").forEach(
      (el) => (el.innerHTML = data.team2_name.slice(0, 3))
   );
}
